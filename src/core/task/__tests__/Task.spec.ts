      ...
      describe("processUserContentMentions", () => {
        it("should process mentions in user_message tags", async () => {
          ...
          const userContent = [
            ...
            {
              type: "text",
              text: "<usr>Text with 'some/path' (see below for file content) in user_message tags</usr>",
            } as const,
            {
              type: "tool_result",
              tool_use_id: "test-id",
              content: [
                {
                  type: "text",
                  text: "<usr>Check 'some/path' (see below for file content)</usr>",
                },
              ],
          ...
          expect((processedContent[1] as Anthropic.TextBlockParam).text).toContain(
            "<usr>Text with 'some/path' (see below for file content) in user_message tags</usr>",
          )
          ...
          expect((content1 as Anthropic.TextBlockParam).text).toContain(
            "<usr>Check 'some/path' (see below for file content)</usr>",
          )
  ...
  describe("abortTask", () => {
    ...
    describe("cancelCurrentRequest", () => {
      ...
      it("should be called during dispose", () => {
        ...
        vi.spyOn(task.messageQueueService, "removeListener").mockImplementation(() => task.messageQueueService as any)
