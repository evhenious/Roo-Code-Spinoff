...
export interface ConvertToOpenAiMessagesOptions {
   ...
   * Only applies when using OpenAI-compatible providers that support the developer role.
   * Env_det blocks are identified as text whose trimmed content starts with `<env_det>` and ends with `</env_det>`.
   */
    ...
    */
    if (anthropicMessage.role === "user") {
      const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
        nonToolMessages: (RooTextBlockParam | Anthropic.ImageBlockParam)[]
      ...
      toolMessages.forEach((toolMessage) => {
        // The Anthropic SDK allows tool results to be a string or an array of text and image blocks, enabling rich and structured content.
        // In contrast, the OpenAI SDK only supports tool results as a single string, so we map the Anthropic tool result parts into one concatenated string to maintain compatibility.
        let content: string
      ...
      const filteredNonToolMessages = nonToolMessages.filter(
        (part) => part.type === "image" || (part.type === "text" && part.text?.trim()),
      )

      if (filteredNonToolMessages.length > 0) {
        // Step 1: Always split env_det blocks first when useDeveloperRole is enabled.
        // This takes priority over mergeToolResultText since developer messages
        // should be separate from tool result merging.
        const developerBlocks: RooTextBlockParam[] = []
        const regularBlocks: (RooTextBlockParam | Anthropic.ImageBlockParam)[] = []

        for (const item of filteredNonToolMessages) {
          if (options?.useDeveloperRole && item.type === "text" && typeof item.text === "string") {
            const itemText = item.text.trim() // todo remove trim here?

            // a. pushing <env_det> wrapped parts to developer role blocks
            //   developer role messages are perceived as informational context and will NOT force LLM to react
            //   e.g. sending here error for not used tool is acknowledged by LLM but no action is forced
            const isEnvBlock =
              item._type === "env" || (itemText.startsWith("<env_det>") && itemText.endsWith("</env_det>"))

            if (isEnvBlock) {
              developerBlocks.push({
                ...item,
                text: item.text.replace("<env_det>", "").replace("</env_det>", "").trim(),
              })
              continue
            }

            // b. pushing <usr> wrapped parts to regular blocks - this is what user typed
            if (itemText.startsWith("<usr>") && itemText.endsWith("</usr>")) {
              regularBlocks.push({
                ...item,
                text: item.text.replace("<usr>", "").replace("</usr>", "").trim(),
              })
              continue
            }
          }

          // d. if useDeveloperRole is off or blocks not match - just use regularBlocks as default
          regularBlocks.push(item)
        }

        // Push developer messages (if any)
        if (developerBlocks.length > 0) {
          openAiMessages.push({
            role: "developer",
            content: developerBlocks.map((block) => ({ type: "text" as const, text: block.text })),
          })
        }

        // Step 2: Process remaining regular blocks
        if (regularBlocks.length > 0) {
          ...
          const hasOnlyTextContent = regularBlocks.every((rb) => rb.type === "text")
          ...
          if (shouldMergeIntoToolMessage) {
            ...
            if (lastToolMessage?.role === "tool") {
              const additionalText = regularBlocks.map((rb) => (rb as Anthropic.TextBlockParam).text).join("\n")
            ...
            openAiMessages.push({
              role: "user",
              content: regularBlocks.map((part) => {
                if (part.type === "image") {
                  return {
                    type: "image_url",
                    image_url: { url: `data:${part.source.media_type};base64,${part.source.data}` },
