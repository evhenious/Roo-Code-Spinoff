			...
			try {
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "tool_call_partial": {
								...
								for (const event of events) {
									if (event.type === "tool_call_start") {
										...
										const lastBlock =
											this.deps.assistantMessageContent[
												this.deps.assistantMessageContent.length - 1
											]
							...
							case "text": {
								...
								const lastBlock =
									this.deps.assistantMessageContent[this.deps.assistantMessageContent.length - 1]
								if (lastBlock?.type === "text" && (lastBlock as any).partial) {
				...
				if (hasTextContent || hasToolUses) {
					...
					if (this.deps.apiConversationHistory.length > 0) {
						const lastMessage =
							this.deps.apiConversationHistory[this.deps.apiConversationHistory.length - 1]
