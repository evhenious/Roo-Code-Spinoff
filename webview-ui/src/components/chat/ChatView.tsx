...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = ({ isHidden }, ref) => {
        ...
        case "ask":
          ...
          switch (lastMessage.ask) {
            case "api_req_failed":
              ...
              setSecondaryButtonText(undefined) // t("chat:startNewTask.title")
              break
            ...
            case "completion_result":
              ...
              setEnableButtons(!isPartial)
              setPrimaryButtonText(undefined) // t("chat:startNewTask.title")
              setSecondaryButtonText(undefined)
              break
            case "resume_task":
              setSendingDisabled(false)
  ...
  const isStreaming = useMemo(() => {
    ...
    if (isLastMessagePartial) {
      ...
      const lastApiReqStarted = modifiedMessages.findLast((message: ClineMessage) => message.say === "api_req_started")
      ...
      if (text || images.length > 0) {
        ...
        if (sendingDisabled || isStreaming || messageQueue.length > 0 || clineAskRef.current === "command_output") {
          try {
            ...
            console.error(`Failed to queue message: ${error instanceof Error ? error.message : String(error)}`)
          }
  ...
  const visibleMessages = useMemo(() => {
    ...
    const newVisibleMessages = modifiedMessages.filter((message) => {
      ...
      if (everVisibleMessagesTsRef.current.has(message.ts)) {
        const alwaysHiddenOnceProcessedAsk: ClineAsk[] = ["api_req_failed", "resume_task", "resume_completed_task"]
  ...
  const handleSuggestionClickInRow = useCallback(
    (suggestion: SuggestionItem, event?: React.MouseEvent) => {
      ...
      // Check if we need to switch modes (UPD it's old - we don't switch modes here)
      // TODO - remove?
      if (suggestion.mode) {
  ...
  useImperativeHandle(ref, () => ({
    acceptInput: () => {
      ...
      } else if (!sendingDisabled && hasInput) {
  ...
  return (
    <div
      ...
      {task ? (
        ...
        <div className="flex flex-col h-full justify-center py-6 px-2 min-h-0 overflow-y-auto gap-4 relative">
