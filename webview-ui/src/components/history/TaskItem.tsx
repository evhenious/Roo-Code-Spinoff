...
const TaskItem = ({
  ...
  const innerHtmlHighlight = !item.title && item.highlight ? { __html: item.highlight } : undefined
      ...
      <div className={(!isCompact && isSelectionMode ? "pl-3 pb-3" : "pl-4") + " flex gap-3 px-3 pt-3 pb-1"}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-1">
            <div
              className={cn(
                "flex-1 min-w-0 overflow-hidden whitespace-pre-wrap font-normal text-ellipsis line-clamp-3",
                {
                  "text-base": !isCompact,
                },
                !isCompact && isSelectionMode ? "mb-1" : "",
              )}
              dangerouslySetInnerHTML={innerHtmlHighlight}
