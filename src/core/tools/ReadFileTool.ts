...
export class ReadFileTool extends BaseTool<"read_file"> {
  ...
  private async executeNew(params: ReadFileParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
    ...
    try {
      ...
      const { maxImageFileSize = DEFAULT_MAX_IMAGE_FILE_SIZE_MB, maxTotalImageSize = DEFAULT_MAX_TOTAL_IMAGE_SIZE_MB } =
        state ?? {}
      ...
      for (const fileResult of fileResults) {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            ...
            await task.renderUIMessage("error", `Error reading file ${relPath}: ${errorMsg}`)
            continue
          }
  ...
  private async handleBinaryFile(
    ...
    if (isSupportedImageFormat(fileExtension)) {
      try {
        ...
        await task.renderUIMessage("error", `Error reading image file ${relPath}: ${errorMsg}`)
        return
      }
    }
    ...
    if (supportedBinaryFormats && supportedBinaryFormats.includes(fileExtension)) {
      try {
        ...
        await task.renderUIMessage("error", `Error extracting text from ${relPath}: ${errorMsg}`)
        return
      }
    }
  ...
  private async requestApproval(
    ...
    if (filesToApprove.length > 1) {
      ...
      if (response === "yesButtonClicked") {
        if (text) await task.renderUIMessage("user_feedback", text, images)
