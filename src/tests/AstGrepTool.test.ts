...
describe("AstGrepTool", () => {
	...
	describe("execute", () => {
		...
		it("should execute sg command with correct parameters", async () => {
			...
			await tool.execute(
				{ query: "console.log($$)", path: "src", file_pattern: "*.ts", lang: "typescript" },
				mockTask as Task,
				mockCallbacks,
			)
			...
			expect(mockSpawn).toHaveBeenCalledWith(
				"sg",
				expect.arrayContaining(["run", "--pattern", "console.log($$)"]),
				expect.any(Object),
			)
		...
		it("should include optional parameters in command", async () => {
			...
			await tool.execute(
				{ query: "test", path: "src", file_pattern: "*.ts", lang: "typescript" },
				mockTask as Task,
				mockCallbacks,
			)

			const spawnCallArgs = mockSpawn.mock.calls[0]?.[1] as string[]
			expect(spawnCallArgs).toContain("run")
			expect(spawnCallArgs).toContain("--pattern")
			expect(spawnCallArgs).toContain("test")
			expect(spawnCallArgs).toContain("src")
			expect(spawnCallArgs).toContain("--globs")
			expect(spawnCallArgs).toContain("*.ts")
			expect(spawnCallArgs).toContain("--lang")
			expect(spawnCallArgs).toContain("typescript")
