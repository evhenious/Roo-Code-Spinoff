// pnpm --filter roo-distilled test core/webview/__tests__/ClineProvider.spec.ts
...
describe("ClineProvider", () => {
  ...
  it("saves current config when switching to mode without config", async () => {
    ...
    ;(provider as any).providerSettingsManager = {
      ...
      listConfig: vi.fn().mockResolvedValue([{ name: "current-config", id: "current-id", apiProvider: "anthropic" }]),
      setModeConfig: vi.fn(),
    } as any
  ...
  describe("deleteMessage", () => {
    ...
    test("handles deletion with confirmation dialog", async () => {
      ...
      expect(mockCline.overwriteClineMessages).toHaveBeenCalledWith([mockMessages[0], mockMessages[1], mockMessages[2]])
  ...
  describe("createTaskWithHistoryItem mode validation", () => {
    ...
    test("preserves mode when it exists in custom modes", async () => {
      ...
      ;(provider as any).providerSettingsManager = {
        ...
        listConfig: vi.fn().mockResolvedValue([{ name: "test-config", id: "config-id", apiProvider: "anthropic" }]),
        activateProfile: vi.fn().mockResolvedValue({ name: "test-config", id: "config-id", apiProvider: "anthropic" }),
      }
