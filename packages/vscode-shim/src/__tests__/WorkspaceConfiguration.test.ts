...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("roo-distilled", context)
			...
			setRuntimeConfig("roo-distilled", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("roo-distilled", "testSetting", "testValue")

			expect(getRuntimeConfig("roo-distilled.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("roo-distilled", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("roo-distilled.setting1")).toBe("value1")
			expect(getRuntimeConfig("roo-distilled.setting2")).toBe(42)
			expect(getRuntimeConfig("roo-distilled.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("roo-distilled.defined")).toBe("value")
			expect(getRuntimeConfig("roo-distilled.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("roo-distilled", "setting1", "value1")
			setRuntimeConfig("roo-distilled", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("roo-distilled.setting1")).toBeUndefined()
			expect(getRuntimeConfig("roo-distilled.setting2")).toBeUndefined()
		})
