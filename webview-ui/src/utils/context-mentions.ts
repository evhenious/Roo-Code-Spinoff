...
/**
 * Gets the description for a mode, prioritizing description > roleDefinition
 * and taking only the first line
 */
function getModeDescription(mode: ModeConfig): string {
  return (mode.description || mode.roleDefinition).split("\n")[0]
}
...
export function insertMention(
  ...
  if (lastAtIndex !== -1) {
    ...
    const afterCursorContent = /^[a-zA-Z0-9\s]*$/.test(afterCursor) ? afterCursor.replace(/^[^\s]*/, "") : afterCursor
