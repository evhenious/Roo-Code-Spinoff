...
const ApiOptions = ({
  ...
  const onProviderChange = useCallback(
    (value: ProviderName) => {
      ...
      if (config) {
        validateAndResetModel(value, apiConfiguration[config.field] as string | undefined, config.field, config.default)
      }
    },
              ...
              <CollapsibleContent className="space-y-3">
                ...
                {selectedProvider === "poe" && (
                  <VSCodeTextField
                    ...
                    className="w-full">
                    <label className="block font-medium mb-1">{t("settings:providers.poeBaseUrl")}</label>
                  </VSCodeTextField>
                )}
                {selectedProvider === "openrouter" &&
