...
describe("ChatTextArea", () => {
  ...
  describe("multi-file drag and drop", () => {
    ...
    describe("prompt history navigation", () => {
      ...
      it("should reset history navigation when sending message", () => {
        ...
        const setInputValue = vi.fn()
        const { container } = render(
          <ChatTextArea {...defaultProps} onSend={onSend} setInputValue={setInputValue} inputValue="Test message" />,
        )
      ...
      it("should reset navigation position when switching between history sources", () => {
        const setInputValue = vi.fn()
        const { rerender } = render(<ChatTextArea {...defaultProps} setInputValue={setInputValue} inputValue="" />)
  ...
  describe("slash command highlighting", () => {
    ...
    it("should not highlight invalid slash commands", () => {
      ...
      expect(highlightLayer.innerHTML).not.toContain('<mark class="mention-context-textarea-highlight">/invalid</mark>')
    ...
    it("should highlight only the command portion, not arguments", () => {
      ...
      expect(highlightLayer.innerHTML).toContain('<mark class="mention-context-textarea-highlight">/deploy</mark>')
    ...
    it("should be case-sensitive when matching commands", () => {
      ...
      expect(highlightLayer.innerHTML).not.toContain('<mark class="mention-context-textarea-highlight">/Setup</mark>')
  ...
  describe("send button visibility", () => {
    it("should show send button when there are images but no text", () => {
      ...
      const sendButton = Array.from(buttons).find((button) => button.querySelector(".lucide-send-horizontal") !== null)
    ...
    it("should show send button when there is both text and images", () => {
      const { container } = render(
        <ChatTextArea {...defaultProps} inputValue="Some text" selectedImages={["data:image/png;base64,test1"]} />,
      )
