...
const extension = vscode.extensions.getExtension<RooCodeAPI>("evhenious.roo-distilled")
...
**NOTE:** To ensure that the `evhenious.roo-distilled` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["evhenious.roo-distilled"]
```
