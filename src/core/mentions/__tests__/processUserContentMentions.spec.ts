...
describe("processUserContentMentions", () => {
  ...
  describe("content processing", () => {
    it("should process text blocks with <usr> tags", async () => {
      const userContent = [
        {
          type: "text" as const,
          text: "<usr>Do something</usr>",
        },
      ]
      ...
      expect(result.content[0]).toEqual({
        type: "text",
        text: "parsed: <usr>Do something</usr>",
      })
    ...
    it("should process tool_result blocks with string content", async () => {
      ...
      expect(result.content[0]).toEqual({
        ...
        content: [
          {
            type: "text",
            text: "parsed: <usr>Tool feedback</usr>",
          },
        ],
      })
      expect(result.mode).toBeUndefined()
    ...
    it("should process tool_result blocks with array content", async () => {
      ...
      expect(result.content[0]).toEqual({
        ...
        content: [
          {
            type: "text",
            text: "parsed: <usr>Array task</usr>",
          },
          {
            type: "text",
            text: "Regular text",
    ...
    it("should handle mixed content types (text + image)", async () => {
      ...
      expect(result.content[0]).toEqual({
        type: "text",
        text: "parsed: <usr>First task</usr>",
      })
  ...
  describe("showRooIgnoredFiles parameter", () => {
    ...
    it("should respect showRooIgnoredFiles when explicitly set to false", async () => {
      const userContent = [
        {
          type: "text" as const,
          text: "<usr>Test explicit false</usr>",
        },
      ]
