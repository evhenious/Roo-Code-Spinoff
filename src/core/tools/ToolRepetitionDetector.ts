...
export class ToolRepetitionDetector {
  ...
  private serializeToolUse(toolUse: ToolUse): string {
    const toolObject: Record<string, any> = {
      ...
      // 'notify' has no params, so any two calls are the same. Need to
      // capture id, as when the tool is "emulated", id contains timestamp when tool called,
      // so breaks similarity immediately
      ...(toolUse.name === "notify" ? { id: toolUse.id } : {}),
    }
