...
describe("RetryQueue", () => {
	...
	describe("enqueue", () => {
		it("should add a request to the queue", async () => {
			...
			await retryQueue.enqueue(url, options, "api-call")
		...
		it("should enforce max queue size with FIFO eviction", async () => {
			...
			// Add 4 requests
			for (let i = 1; i <= 4; i++) {
				await retryQueue.enqueue(`https://api.example.com/test${i}`, { method: "POST" }, "api-call")
			}
	...
	describe("persistence", () => {
		it("should persist queue to workspace state", async () => {
			await retryQueue.enqueue("https://api.example.com/test", { method: "POST" }, "api-call")
	...
	describe("clear", () => {
		it("should clear all queued requests", async () => {
			await retryQueue.enqueue("https://api.example.com/test1", { method: "POST" }, "api-call")
	...
	describe("getStats", () => {
		it("should return correct statistics", async () => {
			...
			await retryQueue.enqueue("https://api.example.com/test1", { method: "POST" }, "api-call")
			await retryQueue.enqueue("https://api.example.com/test2", { method: "POST" }, "api-call")
			await retryQueue.enqueue("https://api.example.com/test3", { method: "POST" }, "api-call")

			const stats = retryQueue.getStats()

			expect(stats.totalQueued).toBe(3)
			expect(stats.byType["api-call"]).toBe(2)
			expect(stats.byType["api-call"]).toBe(1)
	...
	describe("auth state management", () => {
		...
		it("should handle multiple user transitions correctly", async () => {
			...
			await retryQueue.enqueue("https://api.example.com/user1-req", { method: "POST" }, "api-call")
			...
			await retryQueue.enqueue("https://api.example.com/user2-req", { method: "POST" }, "api-call")
			...
			await retryQueue.enqueue("https://api.example.com/user2-req2", { method: "POST" }, "api-call")
	...
	describe("retryAll", () => {
		...
		it("should not retry on 400/404/422 client errors", async () => {
			...
			for (const error of clientErrors) {
				await retryQueue.enqueue(`https://api.example.com/test-${error.status}`, { method: "POST" }, "api-call")
