...
function formatFileReadResult(filePath: string, result: ExtractTextResult): string {
  ...
  if (result.wasTruncated && result.linesShown) {
    ...
    return `${header}
IMPORTANT: Showing lines ${start}-${end} of ${result.totalLines} total lines.
...
async function getFileOrFolderContentWithMetadata(
  ...
  const endsWithLines = mentionPath.match(/:\d{1,}-\d{1,}$/)?.[0]

  let linesRequired: number[] | null = null
  if (endsWithLines) {
    linesRequired = mentionPath.split(":").pop()?.split("-").map(Number) ?? null // kkk.txt:21-22 => 21-22 => [21, 22]
    linesRequired && (linesRequired[0] = linesRequired[0] - 1) // eslint-disable-line
    mentionPath = mentionPath.replace(/:\d{1,}-\d{1,}$/, "")
  }
