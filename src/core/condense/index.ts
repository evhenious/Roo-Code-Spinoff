...
export async function summarizeConversation(options: SummarizeConversationOptions): Promise<SummarizeResponse> {
  ...
  if (messagesToSummarize.length <= 1) {
    const error =
      messages.length <= 1 ? t("common:errors.condense_not_enough_messages") : t("common:errors.condensed_recently")
...
${commandBlocks}
  ...
  // This ensures we account for wrapper text (## Conversation Summary, <system-reminder>, <env_det>)
