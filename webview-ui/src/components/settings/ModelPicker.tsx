...
export const ModelPicker = ({
  ...
  return (
    <>
        ...
        <Popover open={open} onOpenChange={onOpenChange}>
          ...
          <PopoverContent className="p-0 w-[var(--radix-popover-trigger-width)]">
            <Command>
              ...
              <CommandList>
                <CommandEmpty>
                  {searchValue && <div className="py-2 px-1 text-sm">{t("settings:modelPicker.noMatchFound")}</div>}
                </CommandEmpty>
                <CommandGroup>
                  {modelIds.map((model) => (
                    <CommandItem key={model} value={model} onSelect={onSelect} data-testid={`model-option-${model}`}>
                      <span className="truncate" title={model}>
                        {model}
                      </span>
                      <Check
                        className={cn("size-4 p-0.5 ml-auto", model === displayValue ? "opacity-100" : "opacity-0")}
                      />
                    </CommandItem>
                  ))}
                </CommandGroup>
      ...
      {selectedModelInfo?.deprecated && <ApiErrorMessage errorMessage={t("settings:validation.modelDeprecated")} />}
